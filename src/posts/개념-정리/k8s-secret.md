...
title: Secret
date: 2025-07-09
tags: [k8s, configuration, secret, security]
summary: Base64인데도 ConfigMap보다 안전한 이유, 한계와 보완 전략, External Secrets Operator로 외부 저장소와 연동하는 방법
order: 44
---
...
1. **TLS 전송 보호** : Secret은 API 서버와 etcd, API 서버와 kubelet 사이에서 **TLS**로 전송됩니다. 네트워크 스니핑으로 평문이 노출되지 않습니다.
...
6. **읽기 전용 마운트와 파일 권한** : 볼륨 마운트는 읽기 전용이고, `defaultMode`를 `0400`으로 좁히면 컨테이너 프로세스에만 읽기 권한이 주어져 외부 실행이나 라이브 디버깅 도구의 쓰기 변경을 막습니다.
...
1. **ExternalSecret CR**을 작성해 클러스터 내부에 저장할 **목표 Secret 이름**과 **외부 Key** 매핑을 정의.
