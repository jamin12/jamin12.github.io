...
date: 2026-02-17
tags: [elasticsearch, fuzziness, multi_match, highlight, multi-field, autocomplete]
---
...
![multi_match 필드 가중치](./images/15-multi-match-boost.png)
...
![Multi Field 저장 구조](./images/12-multi-field.png)
...
`search_as_you_type`으로 필드를 만들면 내부적으로 네 가지 서브 필드가 생성된다.
...
"프리미엄 감귤 선물 세트"라는 값이 들어가면, 각 서브 필드에 다음과 같이 저장된다.

- **기본 필드** — `[프리미엄, 감귤, 선물, 세트]`
- **_2gram** — `[프리미엄 감귤, 감귤 선물, 선물 세트]`
- **_3gram** — `[프리미엄 감귤 선물, 감귤 선물 세트]`
- **_index_prefix** — `[프, 프리, 프리미, ..., 감, 감귤, 선, 선물, 세, 세트]`

![search_as_you_type 토큰 구조](./images/13-search-as-you-type.png)
...
도큐먼트를 삽입하면, `name` 필드의 서브 필드들에 위에서 설명한 토큰들이 자동으로 생성된다.
...
**bool_prefix**의 핵심은 마지막 단어를 접두사(prefix)로 처리한다는 점이다. "프리미엄 감"으로 검색하면 "프리미엄"은 기본 필드에서 정확히 매칭하고, 마지막 단어 "감"은 `_index_prefix`에서 접두사 매칭하여 "감귤"이 포함된 도큐먼트를 찾는다. 영어에서도 동일하게 동작한다. "you have th"로 검색하면 "you"와 "have"는 정확한 토큰 매칭을 하고, "th"는 "th"로 시작하는 모든 토큰("the", "this", "that" 등)과 매칭된다.

### _2gram, _3gram을 함께 검색하는 이유

기본 필드만으로도 검색 자체는 가능하다. _2gram과 _3gram 필드를 함께 검색하는 이유는 **연속된 단어가 일치할수록 Score를 높여** 더 관련성 높은 결과를 상위에 노출하기 위해서다. "프리미엄 감"으로 검색하면 _2gram 필드에 "프리미엄 감귤"이라는 토큰이 있는 도큐먼트가, 단어 순서가 다른 도큐먼트보다 더 높은 Score를 받는다.
