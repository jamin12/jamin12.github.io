...
Kafka의 설정 파라미터는 수백 개다. 이름은 필요할 때 문서에서 찾으면 되고, 실제로 걸리는 건 그 값을 **어디에 써야 하고, 재기동이 필요하고, 이미 만들어진 토픽에는 적용되는가**다.

## 서버가 갖는 설정과 클라이언트가 갖는 설정

설정은 크게 네 레벨이고, 이 넷은 다시 서버 쪽과 클라이언트 쪽으로 갈린다.
...
나눈 이유는 브로커 재기동이 프로세스 재시작으로 끝나지 않는 데 있다.
...
**두 레벨의 이름은 서로 다르다.** 브로커 쪽은 대체로 `log.` 접두어가 붙고, 최대 메시지 크기처럼 단어 순서가 아예 뒤집힌 경우도 있다. 검색해서 찾은 파라미터명이 어느 레벨의 것인지 먼저 확인하지 않으면 엉뚱한 곳에 값을 넣게 된다.
...
토픽 override는 **한 번 걸리면 계속 남는다.** 나중에 브로커 기본값을 바꿔도 override가 걸린 토픽은 따라오지 않는다. 그래서 원복은 "브로커와 같은 값을 다시 써넣는" 방식이 아니라 **토픽의 override를 삭제하는** 방식이어야 한다. 삭제해야 다시 브로커 기본값을 따라간다. 같은 값을 써넣으면 지금은 같아 보여도 다음에 기본값이 바뀔 때 어긋난다.
...
그래서 설정을 다룰 때는 어디에 무엇을 썼는지 기억하는 것보다 **지금 적용된 값을 조회해 확인하는** 편이 확실하다. 설정을 바꾼 직후와, 이상 동작을 조사할 때 양쪽 모두에서.
