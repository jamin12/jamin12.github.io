...
title: StatefulSet
date: 2025-06-24
tags: [k8s, workload, statefulset]
summary: 고정된 이름, 순서, 파드별 스토리지가 필요한 워크로드를 위한 StatefulSet이 왜 필요한지
order: 14
---
...
StatefulSet으로 배포된 파드는 아래와 같이 **순서가 부여된 고정 이름**으로 생성됩니다.
...
StatefulSet은 각 파드가 다음을 보장해야 할 때 필수적인 오브젝트입니다.
...
4. **파드별로 영구 스토리지(PVC) 고정**
    - 파드가 재생성되더라도 같은 PVC에 연결되어 데이터 유실 없이 안전하게 서비스합니다
