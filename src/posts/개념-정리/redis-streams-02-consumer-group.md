...
직접 읽기 방식인 `XREAD`는 본질적으로 **broadcast**다. 위치를 각 클라이언트가 알아서 관리하니, 모두가 모든 메시지를 본다. 모니터링이나 단일 구독에는 맞지만, "여러 일꾼이 한 작업 목록을 **나눠서** 처리하고, 실패한 건 챙긴다"에는 안 맞는다.
...
워커는 `XREADGROUP`으로 "그룹 이름 + 내 이름"을 대면서 "아직 아무도 안 가져간 새 메시지 달라"고 `>`로 요청한다. 그러면 서버가 알아서 안 겹치게 하나씩 떼어준다.

## 메시지의 생명주기: 받는 것과 끝내는 것은 다르다

Consumer Group의 진짜 핵심은 여기다. 메시지를 받은 상태인 **delivered**와 처리를 끝낸 상태인 **acknowledged**가 분리되어 있다.

워커가 메시지를 받으면, 그 메시지는 **PEL**, 곧 Pending Entries List라는 "처리 중 목록"에 들어간다. 처리를 끝내고 `XACK`를 보내야 비로소 PEL에서 빠진다.
...
만약 PEL 없이 "받는 순간 끝"으로 처리했다면, 워커가 죽는 순간 그 메시지는 증발했을 것이다. PEL이 있어서 최소 한 번은 처리한다는 **at-least-once** 보장이 가능해진다.

아직 ACK되지 않은 처리 중 메시지 목록은 `XPENDING`으로 들여다본다. 누가 받았는지, idle time으로 얼마나 오래 붙잡고 있는지, 몇 번 전달됐는지가 다 나온다 — 막힌 메시지를 찾는 도구다.
