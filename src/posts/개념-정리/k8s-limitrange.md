...
title: LimitRange
date: 2025-07-06
tags: [k8s, resource, namespace]
summary: 컨테이너, 파드, PVC 하나하나의 요청과 제한에 최소, 최대, 기본값을 강제하는 LimitRange
order: 41
---
...
|상황|해결책|
|---|---|
|컨테이너가 `requests` 0, `limits` 무한대로 설정되어 노드 자원 고갈|**min**/ **max**로 하한, 상한 설정|
|야생 YAML에 `requests`를 빠뜨려서 스케줄러 우선순위 꼬임|**defaultRequest**로 자동 주입|
|PVC 하나가 500 Gi를 요청해서 스토리지 폭주|PVC 타입 LimitRange로 **max.storage** 제한|
