...
Kafka에 메시지를 보낸다고 하면 흔히 문자열 한 줄을 떠올린다. 하지만 Kafka가 실제로 다루는 단위는 문자열이 아니라 **레코드**라는 구조체다. 그리고 그 레코드들은 파티션이라는 append-only 로그에 쌓이는데, 이 로그도 하나의 거대한 파일이 아니라 **여러 조각으로 나뉘어 디스크에 저장**된다. 이 글은 두 계층을 잇는다 — 논리적으로 레코드 하나가 무엇으로 이루어져 있고, 물리적으로 그것들이 디스크에 어떻게 쌓이는가.
...
지금까지 레코드 하나를 단위로 봤지만, 프로듀서가 실제로 네트워크에 내보내고 브로커가 디스크에 쓰는 단위는 레코드 하나가 아니다. 여러 레코드를 묶은 **레코드 배치**다.
...
대가는 지연이다. 모아서 보낸다는 건 첫 레코드가 만들어진 뒤 배치가 찰 때까지 기다린다는 뜻이다. 배치를 크게 잡으면 처리량과 압축률은 오르지만 개별 메시지의 지연은 늘어난다. Kafka가 높은 처리량을 내는 이유이자, 메시지를 보냈는데 곧바로 보이지 않는 이유이기도 하다.
...
디스크에서 파티션은 디렉토리 하나이고, 세그먼트는 그 안의 파일이다. 파일 이름은 임의로 붙지 않는다 — **그 세그먼트에 담긴 첫 레코드의 offset을 20자리로 채운 값**이다. 이 첫 offset을 base offset이라 부른다. offset 0부터 시작하는 세그먼트는 `00000000000000000000`, 1000부터 시작하는 세그먼트는 `00000000000000001000`이 된다. 자리를 채우는 이유는 이름순 정렬이 곧 offset 순 정렬이 되게 하기 위해서다. 덕분에 파일을 열어보지 않고 **이름만으로 어느 offset이 어느 세그먼트에 있는지** 알 수 있다.
...
전체 로그를 처음부터 훑는 일은 어느 단계에도 없다. 세그먼트로 쪼개고, 파일 이름에 base offset을 박고, 인덱스를 두는 이유가 여기 있다 — 읽기와 삭제를 국소적으로 만들기 위해서다.
...
key의 value를 null로 보내면 그 key를 지우라는 신호가 된다. 이를 **tombstone**이라 하고, compaction 때 해당 key가 통째로 사라진다. 삭제도 결국 "그 key의 최신값이 '없음'"이라는 레코드로 표현하는 셈이다.

보관 정책은 토픽마다 고른다 — 로그를 기간만큼 쌓다 버리는 `delete`로 갈지, key별 최신값만 유지하는 `compact`로 갈지. 이벤트 스트림인지 상태 저장소인지에 따라 갈리고, 그 선택이 그대로 이 토픽이 "무엇을 기억하는가"를 정한다.
...
배치는 토픽이 아니라 프로듀서 쪽에서 정한다.

| 설정 | 의미 | 예시 값 |
|---|---|---|
| `batch.size` | 배치 하나의 최대 크기(바이트) | `16384` (16KB) |
| `linger.ms` | 배치가 덜 찼어도 이 시간이 지나면 전송 | `5` (기본값) |
| `compression.type` | 배치 압축 방식 | `none` / `gzip` / `snappy` / `lz4` / `zstd` |

`linger.ms`를 낮추면 모을 기회가 줄어 지연은 짧고 배치는 작아진다. 값을 키우면 반대가 된다. 기본값은 Kafka 4.0에서 `0`에서 `5`로 올라갔다.
