...
지금까지 레코드 하나를 단위로 봤지만, 프로듀서가 실제로 네트워크에 내보내고 브로커가 디스크에 쓰는 단위는 레코드 하나가 아니다. 여러 레코드를 묶은 **레코드 배치(record batch)**다.
...
디스크에서 파티션은 디렉토리 하나이고, 세그먼트는 그 안의 파일이다. 파일 이름은 임의로 붙지 않는다 — **그 세그먼트에 담긴 첫 레코드의 offset(base offset)을 20자리로 채운 값**이다. offset 0부터 시작하는 세그먼트는 `00000000000000000000`, 1000부터 시작하는 세그먼트는 `00000000000000001000`이 된다. 자리를 채우는 이유는 이름순 정렬이 곧 offset 순 정렬이 되게 하기 위해서다. 덕분에 파일을 열어보지 않고 **이름만으로 어느 offset이 어느 세그먼트에 있는지** 알 수 있다.
