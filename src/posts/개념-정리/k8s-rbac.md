...
summary: Role, ClusterRole, RoleBinding, ClusterRoleBinding의 구성과 apiGroup의 의미, ServiceAccount와 휴먼 사용자 계정 발급 절차
order: 39
---
...
![Role, Subject, RoleBinding의 관계](./images/k8s-20250706120253.png)
...
# 2. 클러스터 CA로 CSR 서명 (control-plane 노드)
...
* **CN(Common Name)**은 사용자 이름(`alice`)
* **O(Organization)**은 RBAC에서 `subjects.kind: Group` 으로도 활용 가능(`dev-team`)
...
2. IdP에 'kubernetes' 클라이언트를 등록하면 사용자 로그인 시 **ID 토큰(JWT)** 발급
