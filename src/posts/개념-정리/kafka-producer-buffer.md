...
이 값은 파티션당 배치 개수가 아니라 **브로커 커넥션당 요청 개수**다. 5면 응답을 못 받은 요청 5개가 한 브로커를 향해 동시에 날아가 있을 수 있고, 같은 파티션의 배치 1·2·3이 서로 다른 요청에 실릴 수 있다. 그 상태에서 1번이 실패해 재시도되면 2·3이 먼저 기록된다. 파티션 안의 순서가 프로듀서 쪽에서 어긋나는 경로가 이것이고, `enable.idempotence`가 배치에 시퀀스 번호를 붙여 이걸 막는다. Kafka 3.0부터 기본 활성이다.
