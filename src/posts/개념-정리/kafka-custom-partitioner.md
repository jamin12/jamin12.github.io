...
프로듀서가 레코드를 어느 파티션에 넣을지는 브로커가 아니라 프로듀서가 정한다. 그 규칙을 직접 짜려면 `Partitioner`를 구현해 `partitioner.class`에 등록한다. 추상 메서드가 둘뿐인 인터페이스라 구현은 짧고 등록은 설정 한 줄이다. 그 한 줄이 비활성화하는 것은 프로듀서 버퍼 안쪽에서 도는 별도 계층이다.
...
`Partitioner`가 받는 인자는 레코드 하나와 클러스터 메타데이터다. 그 레코드가 몇 번째인지, 지금까지 어느 파티션에 얼마나 보냈는지는 인자에 없다. `BuiltInPartitioner`는 그 진도를 `stickyPartitionInfo`와 `partitionLoadStats`라는 필드로 들고 있다.
...
`send()`는 레코드를 곧바로 내보내지 않고 `doSend()`로 들어간다. 거기서 key와 value를 직렬화하고, 파티션 번호를 정하고, 그 번호로 `RecordAccumulator.append()`를 불러 배치에 넣는다. 가운데 단계를 맡는 것이 `KafkaProducer`의 private 메서드 `partition()`이다.
...
private int partition(ProducerRecord<K, V> record, byte[] serializedKey,
    ...
    if (serializedKey != null && !partitionerIgnoreKeys) {
        return BuiltInPartitioner.partitionForKey(
                serializedKey, cluster.partitionsForTopic(record.topic()).size());
    } else {
...
분기가 넷이고 위에서부터 순서대로 걸린다. 레코드에 파티션 번호를 직접 박아 두면 그 값이 그대로 나가고, 다음이 커스텀 파티셔너, 다음이 key 해싱, 마지막이 남은 경우다.

key 해싱과 그 아래 분기는 `partitionerPlugin`이 비었을 때만 닿는다. 커스텀 파티셔너를 등록하면 `partitionForKey()`도, sticky 배분도, 적응형 배분도 실행되지 않는다. 이 슬롯은 레코드 단위가 아니라 프로듀서 단위로 켜지고 꺼져서, "이 레코드는 내장 로직에 맡기고 저 레코드만 내가 정한다"는 형태를 만들 수 없다.

마지막 분기가 돌려주는 `RecordMetadata.UNKNOWN_PARTITION`은 `-1`이다. 파티션이 아직 정해지지 않았다는 표시이고, `append()`가 배치를 채우면서 `BuiltInPartitioner`에게 번호를 받아 그 자리를 메운다. `doSend()`는 `append()`가 끝난 뒤 `getPartition()`으로 실제 번호를 다시 읽어 온다.

커스텀 파티셔너는 같은 표시를 쓸 수 없다. 음수를 반환하면 위의 검사에 걸려 `IllegalArgumentException`이 난다. 내장 계층에 위임하겠다는 `-1`과 잘못 계산한 음수를 구분할 방법이 인터페이스에 없다.

`partitioner.ignore.keys`·`partitioner.adaptive.partitioning.enable`·`partitioner.availability.timeout.ms` 세 값도 커스텀 파티셔너 분기 뒤에서 읽힌다. Kafka 문서가 셋 모두에 커스텀 파티셔너를 쓰면 이 설정은 효과가 없다고 적어 둔 것이 같은 이야기다. 설정을 조회하면 값은 그대로 나오지만 그 값을 읽는 코드에 닿지 않는다.

## 꺼지는 것은 누적 바이트로 파티션을 고르는 계층이다

key가 있으면 파티션은 `murmur2(keyBytes) % 파티션수`로 정해지고, 이 계산은 2.x부터 지금까지 같다. 갈리는 쪽은 key가 `null`일 때다. 해싱할 것이 없으니 다른 기준이 필요하고, 그 자리를 sticky 배분이 맡는다.
...
2.4에서 처음 들어온 sticky는 옮겨 갈 시점을 배치 완성으로 잡았다. 여기에 편향이 있었다. 브로커가 느리면 그 파티션의 큐에서 레코드가 대기하다 배치 하나에 더 많이 담기므로, 파티션별 배치 개수가 비슷해도 레코드는 느린 브로커로 더 몰린다.
...
점선이 내장 경로에만 붙는다. Sender는 배치를 꺼내 갈 브로커를 고르면서 파티션마다 얼마나 밀려 있는지를 세어 `updatePartitionLoadStats()`로 넘기고, sticky는 다음 파티션을 고를 때 그 통계로 가중 랜덤을 뽑아 적체가 심한 브로커의 파티션이 뽑힐 확률을 낮춘다. `partitioner.adaptive.partitioning.enable`이 켜고 끄는 것이 이 경로다. `partition()`은 이 통계를 볼 방법이 없다.

그래서 `BuiltInPartitioner`가 들고 있는 두 상태는 쓰는 스레드가 다르고, 필드 modifier가 그 차이를 그대로 드러낸다.
...
이 계층이 `internals`에 있는 이유가 여기서 나온다. `partition()`은 "이 레코드 하나를 어디로 보낼까"라는 질문만 받는데, 균일성을 확보하는 방법은 "이 파티션에 몇 바이트가 쌓였나"를 세는 것이고 그 숫자는 배치를 실제로 채우는 `RecordAccumulator`만 안다. 인터페이스로 공유 버퍼의 내부 상태를 넘길 방법이 없다.
