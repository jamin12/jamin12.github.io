...
title: ConfigMap
date: 2025-07-09
tags: [k8s, configuration, configmap]
summary: 설정을 환경 변수와 볼륨으로 컨테이너에 넣는 방법, 스크립트를 담아 실행하는 방법, 변경 사항을 적용하는 방법
order: 43
---
쿠버네티스(Kubernetes)에서는 컨테이너화된 애플리케이션의 설정을 효과적으로 관리하기 위해 다양한 리소스를 제공합니다. 그중 대표적인 리소스가 바로 **ConfigMap**입니다.
...
3. **실행 권한 부여**: ConfigMap으로 마운트된 파일은 기본적으로 0444(읽기 전용)이므로,
    - `securityContext.fsGroup`을 이용해 전체 파일을 0755 권한으로 바꾸거나,
