---
title: 사이드카, 앰배서더, 어댑터 패턴
date: 2025-07-01
tags: [k8s, pod, container, design-pattern]
summary: 한 파드에 보조 컨테이너를 두는 세 가지 멀티 컨테이너 패턴과 각각의 예시 YAML
order: 37
---
...
| **로그 수집**       | Fluent Bit 사이드카가 애플리케이션 로그 파일을 tail 해서 stdout으로 전송 |
...
Adapter 컨테이너가 주 컨테이너의 stdout 로그를 tail 하여 **JSON에서 Stackdriver 형식**으로 변환, 전송합니다. 이렇게 포맷, 프로토콜을 바꾸는 게 어댑터의 핵심 역할입니다.
...
| **프로토콜 변환**    | gRPC를 REST로, XML을 JSON으로 바꾸는 컨버터를 별도 컨테이너로 구현     |
