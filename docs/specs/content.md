...
| 대상 | 정렬 |
|------|------|
| 글 목록 (전체/카테고리/태그) | `date` 내림차순 (최신순), 같은 날짜면 slug 이름순 |
| 카테고리 목록 | 글 수 내림차순 → 이름 가나다순 |
| 태그 목록 | 사용 빈도 내림차순 |

**기각 기록 (2026-06-23, `order` 수동 순서 필드 제거)**: 한때 frontmatter `order`(정수) 필드를 1순위 정렬키로 두어, 개념 정리의 다부작 글을 1편→N편 순서로 위에서 아래로 고정했다(`order` 오름차순 → 같은 order면 날짜순). 의도는 "순서대로 읽는 묶음"이었으나, 결과적으로 목록 상단에 오래된 1편이 박혀 **카테고리 목록이 최신 글을 안 보여주는** 문제가 생겼다. 순서 있는 묶음은 이미 `series`/`seriesOrder`(→ [series.md](series.md))가 전담하므로 `order`는 역할이 겹쳤다. 결정: `order` 필드를 전 글에서 제거하고 정렬을 순수 날짜 내림차순으로 되돌린다. 시리즈 정렬은 `seriesOrder` 기반이라 영향 없음.
