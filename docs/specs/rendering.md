...
`mermaid.render`는 측정용 DOM을 라이브러리 내부에서 공유한다. 한 글에 다이어그램이 셋 이상 있으면 각 컴포넌트의 effect가 동시에 render를 부르고, 서로의 상태를 덮어써 일부가 빈 SVG로 돌아온다. throw가 아니라 falsy를 돌려주므로 에러 박스도 뜨지 않고 빈 박스만 남는다.

그래서 `MermaidDiagram`은 module-scope 프로미스 큐(`enqueueRender`)로 호출을 한 줄로 세운다. 다이어그램 넷이 있는 글에서 넷 중 둘만 그려지는 것을 확인하고 넣었다.

| 후보 | 결과 |
|------|------|
| 컴포넌트마다 그대로 동시 호출 | 셋 이상이면 일부가 조용히 빈 박스 |
| **module-scope 큐로 직렬화** | **채택.** 다이어그램 수만큼 순차 렌더. 한 개당 수십 ms라 체감 지연 없음 |
| `mermaid.run()`으로 일괄 렌더 | DOM을 라이브러리가 직접 만지므로 React 소유권과 충돌 |

**렌더 실패 시**

mermaid.render가 throw하면 에러 메시지 + 원본 코드를 `.mermaid-diagram--error` 박스로 노출. 무음 실패(그냥 빈 박스) 금지 — 작성자가 문법 오류를 즉시 알 수 있어야 함.

빈 SVG도 실패로 처리한다. `render`가 falsy를 돌려주면 컴포넌트가 직접 throw해서 같은 에러 박스로 보낸다. 이 경로가 없던 동안 동시 호출 문제가 빈 박스로만 드러나 원인을 찾기 어려웠다.
