...
- [x] 글 상세 `/posts/:slug`
- [x] **카테고리 이전/다음 네비** (2026-06-11) — 시리즈 없는 글의 상세 푸터에 같은 카테고리 내 시간순 이전(오래된)/다음(최신) 글 링크. `getCategoryNav(slug)`. 시리즈 글은 기존 시리즈 네비가 우선 (둘 다 노출하면 푸터가 중복으로 어수선). 스타일은 `series-nav` 클래스 공유
