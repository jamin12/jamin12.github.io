---
layout: post
title: "하나의 class로 여러가지의 dto를 받는 방법"
date: 2025-12-13 14:45:00 +0900
categories: [트러블 슈팅]
tags: [json, dto]
---

## api 하나로 여러개의 json객체 받기

회사에서 작업을 하다가 테이블 Join전략을 만나게 되었다.
...
그래서 이렇게 reqeust를 받는 방법을 생각했는데 request를 받을때 하나는 무조건 null이 되고

어떤게 null인지 체크를 한 후 저장하는 로직을 짜야했다.

근데 만약 상속받는 클래스가 두개가 아니고 10개라면?? 더 많다면?? 그때마다 어떤게 null인지 체크를 해주는 로직을 넣어줘야했다.
...
@JsonTypeInfo는 인터페이스나 추상 클래스에 적용할 수 있다, 위 코드처럼 이렇게 설정을 하면 설정한 name을 type에 넣어서 request를 보내주게되면 알아서 매핑을 해준다.

![image](/assets/imgs/posts/하나의 class로 여러가지의 dto를 받는 방법/image1.png)

-   type : "A\_a" -> A\_aRequest.class
-   type : "A\_b" -> A\_bRequest.class
...
getA() 메소드에서 각 객체가 어떤 객체인지 확인을 한 후 그 객체안에서 변환과정을 거쳐 반환하게 된다.
...
사실 response에는 이 어노테이션이 없어도 되긴 하는데 그렇게 되면 swagger에서 이러한 정보를 얻지 못하게되어서 넣는게 좋다고 생각한다.

![image](/assets/imgs/posts/하나의 class로 여러가지의 dto를 받는 방법/image2.png)
...
- **`include`**: 유형 정보를 어디에 포함시킬 것인지를 지정합니다. JsonTypeInfo.As.EXISTING\_PROPERTY는 기존의 속성에 유형 정보를 포함하겠다는 뜻입니다.
...
- JsonTypeInfo.As.WRAPPER\_OBJECT: 유형 정보를 포함하는 새 객체를 생성하여 인스턴스 데이터를 감쌉니다. 예를 들어, 인스턴스가 {"property": "value"}라면, 이 옵션을 사용하면 {"typeValue": {"property": "value"}}와 같이 됩니다.
- **`JsonTypeInfo.As.EXTERNAL_PROPERT`**: 유형 정보는 인스턴스와 같은 수준의 외부 속성으로 취급됩니다. 예를 들어, 인스턴스가 {"property": "value"}이고, property 속성의 값이 "type"이라면, 이 옵션을 사용하면 {"type": "typeValue", "property": "value"}와 같이 됩니다. 하지만 이 경우, 유형 정보와 인스턴스 데이터는 서로 다른 JSON 객체로 취급됩니다.
...
- [https://kdohyeon.tistory.com/99](https://kdohyeon.tistory.com/99)
- [https://www.logicbig.com/tutorials/misc/jackson/json-type-info-with-logical-type-name.html](https://www.logicbig.com/tutorials/misc/jackson/json-type-info-with-logical-type-name.html)